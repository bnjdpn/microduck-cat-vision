...
- `?demoViolation=Jaina:left` (or `Nooby:right`) previews a wrong-feeder
  event after the entrance. A comma-separated pair previews both cats at
  once. This is a deterministic visual-QA hook; normal URLs only react to
  Cat Vision results.

## Cat feeding corner

The arena contains a stylised procedural reconstruction of the fixed feeding
setup: two first-generation Xiaomi Smart Pet Food Feeders around a Petlibro
Dockstream 2. Persistent signs identify `Nooby · gauche` and `Jaina · droite`.
When Cat Vision confirms a wrong-feeder event, a rear-facing coat-coloured cat
marker and alert halo appear at that bowl, and the walking controller steers
toward the corresponding arena coordinates before stopping short of the cat.
