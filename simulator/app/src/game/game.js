...
import { createFeedingCornerScene, FEEDER_ARENA_TARGETS } from "./feeder-scene.js";
...
async function boot({ scene, camera, renderer }) {
  ...
  const feederRunCommand = new Float32Array(3);
  const FEEDER_RUN_TIMEOUT_MS = 3_000;
  let feederRun = null;

  function clearFeederRun() {
    feederRun = null;
    feederRunCommand.fill(0);
  }

  function refreshFeederRun({ label, side, bearing = 0.5 }) {
    const [forwardLimit, , angularLimit] = velLims();
    const target = FEEDER_ARENA_TARGETS[side] ?? null;
    if (!target) {
      const clampedBearing = Math.min(1, Math.max(0, Number(bearing) || 0.5));
      feederRunCommand[0] = forwardLimit;
      feederRunCommand[1] = 0;
      feederRunCommand[2] = Math.min(
        angularLimit,
        Math.max(-angularLimit, (0.5 - clampedBearing) * 2 * angularLimit),
      );
    }
    feederRun = {
      label,
  ...
  function updateFeederRunCommand() {
    if (!feederRun?.target) return;
    const [forwardLimit, , angularLimit] = velLims();
    const qpos = data.qpos;
    // Three.js uses (x, y-up, z) while MuJoCo uses (x, y, z-up).
    const dx = feederRun.target.x - qpos[0];
    const dy = -feederRun.target.z - qpos[1];
    ...
    feederRunCommand[1] = 0;
    if (distance <= 0.20) {
      feederRunCommand[0] = 0;
      feederRunCommand[2] = 0;
      feederRun.arrived = true;
      return;
    }
    feederRun.arrived = false;
    const turnPenalty = Math.min(0.86, Math.abs(yawError) / 1.35);
    feederRunCommand[0] = forwardLimit * Math.max(0.14, 1 - turnPenalty);
    feederRunCommand[2] = Math.min(angularLimit, Math.max(-angularLimit, yawError * 2.1));
  }
  ...
  function effectiveCmd() {
    if (inputLocked || headMode || mode === "roll" || mode === "crouch" ||
      ...
      return ZERO_CMD;
    if (feederRun) {
      if (performance.now() - feederRun.lastObservedAt <= FEEDER_RUN_TIMEOUT_MS) {
        updateFeederRunCommand();
        return feederRunCommand;
      }
      clearFeederRun();
    }
  ...
  const feederScene = createFeedingCornerScene();
  scene.add(feederScene.group);
  ...
  function playChirp(voice = null) {
    const bank = voice?.bank ?? VOICE_BANK[currentVariant] ?? "duck1";
    const take = voice?.take ?? CHIRP_TAKES[(Math.random() * CHIRP_TAKES.length) | 0];
  ...
  const quackLoud = (voice = null) => {
    quackAt = performance.now();
    playChirp(voice);
    stickers?.pop("quack");
  };
  const CAT_NAME_SOUNDS = {
    Nooby: "nooby.wav",
    Jaina: "jaina.wav",
  };
  const catNameCache = new Map();
  const catNameQueue = [];
  let activeCatName = null;

  function playNextCatName() {
    if (activeCatName !== null || catNameQueue.length === 0) return;
    const label = catNameQueue.shift();
    const filename = CAT_NAME_SOUNDS[label];
    const url = signed(`./assets/voices/cats/${filename}`);
    let audio = catNameCache.get(url);
    if (!audio) {
      audio = new Audio(url);
      audio.volume = 1;
      catNameCache.set(url, audio);
    }
    activeCatName = label;
    const finish = () => {
      if (activeCatName !== label) return;
      audio.onended = null;
      audio.onerror = null;
      activeCatName = null;
      window.setTimeout(playNextCatName, 100);
    };
  ...
  function speakCatName(label, { force = false } = {}) {
    if (!CAT_NAME_SOUNDS[label]) return false;
    if (!force && (activeCatName === label || catNameQueue.includes(label))) return true;
    if (force && catNameQueue.includes(label)) return true;
    catNameQueue.push(label);
    playNextCatName();
    return true;
  }
  ...
  // A short vertical impulse gives Nooby a distinct physical reaction
  // without changing the locomotion policy or resetting the simulation.
  function catJump() {
    if (inputLocked || recovery || mode !== "walk") return false;
    const qvel = data.qvel;
    qvel[2] = Math.max(qvel[2], 1.65);
    return true;
  }

  // Webcam observations arrive every 1.2 seconds. The loop is kept alive by
  // each frame that still contains Nooby and stops as soon as a frame no
  // longer contains her (or when the camera is stopped).
  const NOOBY_JUMP_INTERVAL_MS = 1_000;
  const LIVE_OBSERVATION_TIMEOUT_MS = 3_000;
  const observedCats = new Set();
  const observedFeederViolations = new Set();
  let noobyJumpTimer = null;
  let lastLiveObservationAt = -Infinity;

  function stopNoobyJumpLoop() {
    if (noobyJumpTimer !== null) window.clearTimeout(noobyJumpTimer);
    noobyJumpTimer = null;
  }

  function noobyJumpTick() {
    noobyJumpTimer = null;
    const observationExpired = performance.now() - lastLiveObservationAt > LIVE_OBSERVATION_TIMEOUT_MS;
    if (!observedCats.has("Nooby") || observationExpired) {
      if (observationExpired) observedCats.clear();
      return;
    }
    catJump();
    noobyJumpTimer = window.setTimeout(noobyJumpTick, NOOBY_JUMP_INTERVAL_MS);
  }

  function startNoobyJumpLoop() {
    if (noobyJumpTimer === null) noobyJumpTick();
  }

  function clearCatPresence() {
    observedCats.clear();
    observedFeederViolations.clear();
    lastLiveObservationAt = -Infinity;
    stopNoobyJumpLoop();
    clearFeederRun();
    feederScene.setViolations([]);
  }

  function observeFeederViolations(violations, { live = true } = {}) {
    const next = (violations ?? []).filter((violation) =>
      CAT_NAME_SOUNDS[violation?.label] &&
      (violation?.side === "left" || violation?.side === "right")
    );

    if (!live) {
      feederScene.setViolations(next);
      for (const violation of next) speakCatName(violation.label, { force: true });
      return next.length > 0;
    }

    const nextKeys = new Set(next.map((violation) => `${violation.label}:${violation.side}`));
    for (const violation of next) {
      const key = `${violation.label}:${violation.side}`;
      if (!observedFeederViolations.has(key)) {
        // A normal sighting may just have announced the cat. Queue a second,
        // louder-purpose name call for the confirmed wrong-feeder event.
        speakCatName(violation.label, { force: true });
      }
    }
    observedFeederViolations.clear();
    for (const key of nextKeys) observedFeederViolations.add(key);
    feederScene.setViolations(next);

    if (next.length) refreshFeederRun(next[0]);
    else clearFeederRun();
    return next.length > 0;
  }

  function observeCats(labels, { live = true } = {}) {
    const next = new Set((labels ?? []).filter((label) => CAT_NAME_SOUNDS[label]));

    // A selected photo is a single observation, not a live presence. It gets
    // the same audible reaction and one jump for Nooby, without leaving a
    // background loop running forever.
    if (!live) {
      clearCatPresence();
      for (const label of next) speakCatName(label);
      if (next.has("Nooby")) catJump();
      return next.size > 0;
    }

    for (const label of next) {
      if (!observedCats.has(label)) speakCatName(label);
    }
    lastLiveObservationAt = performance.now();
    observedCats.clear();
    for (const label of next) observedCats.add(label);

    if (observedCats.has("Nooby")) startNoobyJumpLoop();
    else stopNoobyJumpLoop();
    return observedCats.size > 0;
  }
  ...
  Object.assign(gameApi, {
    ...
    catJump,
    observeCats,
    observeFeederViolations,
    clearCatPresence,
    startEntrance: () => ceremony.startEntrance(),
    // Kept for callers outside CatVisionPanel: a one-shot observation must
    // never start the continuous webcam loop.
    reactToCat: (label) => observeCats([label], { live: false }),
  });
  ...
  window.rl = {
    ...
    spawnBall, triggerKick, triggerRoll, catJump, observeFeederViolations, sessions, ort,
    feederScene,
    get feederRun() { return feederRun ? { ...feederRun, command: [...feederRunCommand] } : null; },
  ...
  // Opt-in deterministic scene preview for browser QA and demonstrations.
  // Normal URLs never synthesize an observation; live Cat Vision remains
  // the only production source of violations.
  const demoViolation = new URLSearchParams(window.location.search).get("demoViolation");
  if (demoViolation) {
    const demoViolations = demoViolation.split(",").map((entry) => {
      const [label, side] = entry.split(":");
      return { label, side, bearing: side === "left" ? 0.25 : 0.75 };
    }).filter(({ label, side }) => CAT_NAME_SOUNDS[label] && FEEDER_ARENA_TARGETS[side]);
    if (demoViolations.length) {
      void ceremony.entranceFinished.then(() => {
        observeFeederViolations(demoViolations, { live: true });
      });
    }
  }
